...
Currently, the best algorithms for such tasks are based on convolutional neural networks. Performance of convolutional neural networks on the ImageNet tests, is now close to that of humans. The rapid development of CNN lies in the vast utilization of GPU and [large image datasets](https://en.wikipedia.org/wiki/List_of_datasets_for_machine_learning_research#Image_data). There are successful applications in image recognition, natural language processing, and playing [computer Go](https://en.wikipedia.org/wiki/Computer_Go), as well as popular frameworks such as Caffe and Tensorflow.
...
#### Overview
In this project, we study the CNN based image classification with the famous CIFAR-10 datasets [[7]](http://www.cs.toronto.edu/~kriz/cifar.html) and Caffe framework. We show how to refine CNN with two optimized methods, the dropout and fine tuning pre-trained network, to achieve clost to state-of-art accuracy.
...
For this multilabel classification problem, we focus on how much images can be correctly predicted. We expect the algorithm to have good performance (accuracy > 70%), thus the accuracy is choosen for its simpleness. Accuracy is defined by number of true positives divided by total number of testing samples.
...
Softmax loss is used in the last layer of CNN, for the convinience of its probaility intepretation that category with maximum probability should be the predicted label. The full loss for the dataset is the mean of $L_i$ over all training examples together with a regularization term.
...
#### Benchmark
For benchmark, we collect samples on CIFAR10 images classification from Sivan Sabato [8]. The human classification is reported to be 94%, the 47th best score is 79.6%, we will evaluate our final CNN as success in this range.
...
#### Visualization
To gain a better view of possible patterns in the datasets, we performed a PCA feature extraction, and plotted the 2-D feature map of the 1000 images in data sets. The labels from 0 to 9 corresponds to names `['airplane', 'automobile', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']` in order. The axises are reduced 2-D features generated by PCA codes.
...
We will use the default solver Stochastic Gradient Descent, tuning the hyperparameters learning rate `alpha`, momentum `mu`, and learning policy and steps.
...
#### CNN with dropout
For brevity, we will describe the difference of CNN with dropout and fine tunign Caffenet with the original ConvNet, other details can be found in their prototxt files.
...
The learning curves are smooth, with test accuracy continously increasing, training loss and testing loss decreasing. It means that the there is not overfitting problem. Also the gap of two loss curves is not obvious, implies that the variance is small thus the algorithm is quite robost.
...
Due to the probabilistic regularization brought by dropout, during trainging there is large variance for testing loss curve. However, the algorithm mangages to overcome this issue once the learning rate deceases by factor of 10 at iteration 60000.
...
### 4.2 Justification
The three CNNs give successful results for CIFAR10 image classification, with final testing accuracy scores of 81% to 83.55% and 89% seperately.
We also show how the improvement of CNN with dropout and fine-tuning pre-trained CNN. The final result fine-tuning CaffeNet has close to the state-of-art score compared to the previous benchmark, with much less effort in computation.
Thus we have shown that the study clearly demostrated how to design, train, and improve CNN for CIFAR10 image classification task.
...
4. Underfitting: CNN couldn't tell the difference between a SUV and truck, but seems to know it's a vechile, which may be improved by more traning samples.
